      //set item template
      let productTemplate = (prod) => {
        let item = `
        <div class="product">
            <img src="https://cdn.sellix.io/static/images/products/${prod.image_attachment}.jpg">

            <div class="text" >
                <h2>  ${prod.title}</h2>
                <p  class="stock">  Stocks: ${prod.stock}</p>
    ...
    //category template
    let categorieTemplate = (category, index) => {
        let categoryItem = `
            <button data-index=${index} onClick="showProducts()" class="category-item"> ${category.title}</ button>
        `
        return categoryItem;
    }
    ...
    let getProducts = () => {
        ...
        .then(response =>{
            ...
            categories.map((category, index) => {
                $(".categories").append(categorieTemplate(category, index))
            })
    ...
    let showProducts = () => {
        ...
        $('.category-item').removeClass("clicked");


        try{
            index = event.target.getAttribute("data-index");
            $('.category-item[data-index=' + index + ']').addClass("clicked");
        }catch(err){
            index = 0;
            $('.category-item[data-index="0"]').addClass("clicked");
        ...
        let categoriesProducts = categories[index].products_bound;
